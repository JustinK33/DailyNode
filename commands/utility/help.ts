...
export async function execute(interaction) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      'DailyNode now supports server-level and personal settings. Use the commands below to configure your workflow.'
    )
    .addFields(
      {
      ...
      },
      {
        name: '📌 Compatibility',
        value:
          "• `/todayleetcode` still works and shows the server's daily question when available.",
        inline: false,
      }
    )
