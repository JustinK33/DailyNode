...
export function initializeUserReminderScheduler(client, userChallengeService) {
  if (client[USER_SCHEDULER_KEY]) {
    console.warn(
      '[CRON] User reminder scheduler already initialized. Skipping duplicate registration.'
    );
  ...
  const task = cron.schedule(
    ...
    async () => {
      ...
      if (isRunning) {
        console.warn(
          `[CRON] userReminder overlap detected at ${startedAt.toISOString()}; skipping tick.`
        );
        return;
      }
      ...
      try {
        ...
        const result = await userChallengeService.sendDueReminderDMs(
          client,
          new Date()
        );
        ...
        if (result.sentCount > 0) {
          console.log(
            `✅ User reminder scheduler sent ${result.sentCount} DM reminder(s).`
          );
        }
      } catch (error) {
