...
export function initializeServerDailyScheduler(client, serverChallengeService) {
  if (client[SERVER_SCHEDULER_KEY]) {
    console.warn(
      '[CRON] Server daily scheduler already initialized. Skipping duplicate registration.'
    );
  ...
  const task = cron.schedule(
    ...
    async () => {
      ...
      if (isRunning) {
        console.warn(
          `[CRON] serverDaily overlap detected at ${startedAt.toISOString()}; skipping tick.`
        );
        return;
      }
      ...
      try {
        ...
        const result = await serverChallengeService.runDueGuildChallenges(
          client,
          new Date()
        );
        ...
        if (result.sentCount > 0) {
          console.log(
            `✅ Server daily scheduler sent ${result.sentCount} challenge(s).`
          );
        }
      } catch (error) {
